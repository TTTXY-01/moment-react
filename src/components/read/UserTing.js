  ...
  scroll = () => {
    ...
    if (document.body.scrollTop >= document.body.scrollHeight - document.documentElement.clientHeight) {
      this.setState({
        ...
        let interFace = '/ting/listByNJ.php' + location.search + '&pageSize=10&client=web&minId=' + this.state.data[this.state.minIndex].content.contentId
        this.ajaxData(interFace)
      })
    }
  }
  componentDidMount() {
    this.ajaxData('/ting/listByNJ.php' + location.search + '&pageSize=10&client=web&minId=')
